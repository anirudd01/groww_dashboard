...
Two endpoints share the same candle contract, the same intervals and the same per-call limits: one for instruments that are still trading, and one for F&O contracts that have expired. They differ only in how an instrument is identified.

| Request Type | Path                               | Description                             |
|--------------|------------------------------------|-----------------------------------------|
| **GET**      | [`/market/historical/{interval}`](#get-historical-data) | Fetches historical OHLCV data.          |
| **GET**      | [`/market/historical/expired/{interval}`](#get-expired-contract-candles) | Fetches historical OHLCV data for expired F&O contracts. |
...
the two endpoints give a strategy the full price history of a contract, from listing through to
expiry.
...
**NOTE: How a trading symbol is composed**

A symbol carries its underlying, its expiry date and — for options — its strike and side:

```
<UNDERLYING><YYMMMDD><STRIKE><CE|PE>     options
<UNDERLYING><YYMMMDD>FUT                 futures
```

The expiry date is **`YYMMMDD`**: a two-digit year, a three-letter month in uppercase, and a
two-digit day of the month — in that order.

| Trading symbol           | Underlying | Expiry (`YYMMMDD`) | Expiry date  | Strike  | Side |
|--------------------------|------------|--------------------|--------------|---------|------|
| `NIFTY26AUG2523950CE`    | `NIFTY`    | `26AUG25`          | 2026-08-25   | `23950` | `CE` |
| `NIFTY26JUL2825950PE`    | `NIFTY`    | `26JUL28`          | 2026-07-28   | `25950` | `PE` |
| `NIFTY26AUG25FUT`        | `NIFTY`    | `26AUG25`          | 2026-08-25   | —       | —    |

Note the order: the year leads and the day follows the month, so `26AUG25` is the 25th of
August 2026, not the 26th of August 2025. Take symbols from
[Contracts & Expiries](https://api-docs.indstocks.com/contracts/) rather than building them by hand — that is what those
endpoints are for — and use this to read one back.
...
NSE derivatives, using the default segment:
...
the stable identifier for historical contracts and is what this endpoint returns. Pass it as
`trading-symbols` to
[Get Expired Contract Candles](https://api-docs.indstocks.com/historicalData/#get-expired-contract-candles).
...
- [Historical Data](https://api-docs.indstocks.com/historicalData/) — fetch candles for the contracts you find here, including [expired contracts](https://api-docs.indstocks.com/historicalData/#get-expired-contract-candles)
...
Updates are published as a JSON-encoded string. Decode the frame to obtain the string,
then decode that string to obtain the update object.

The example below is an execution update, which carries every field this feed sends.
Updates earlier in the order's life carry a subset; the `When sent` column in the field
table states which.

*   **Example frame:**
    ```text
    "{\"mode\":\"order_update\",\"timestamp\":1789628366550,\"data\":{\"order_id\":\"97603202\",\"entity_name\":\"SENSEX 17 SEP 74400 CE\",\"lot\":1,\"order_type\":\"BUY\",\"order_status\":\"S\",\"executed_price\":306.45,\"elapsed_time\":8,\"error_message\":\" \",\"timestamp\":1789628366544,\"req_quantity\":20,\"requested_lot\":1}}"
    ```
...
|------------------|---------|------------------------|-------------------------------------------------------------------------------|
| `order_id`       | string  | Every update           | Order reference, numeric and sent as a string. Use it to correlate updates with [Get Order Book](https://api-docs.indstocks.com/normal_orders/#get-order-book). |
...
`data` omits a field rather than sending it at its default value, so check that a field is
present before reading it. `elapsed_time` and `error_message` are sent together.
...
An order publishes a sequence of updates as it progresses. A market order that fills moves
through `R` → `P` → `S`:
...
**NOTE: Sequencing updates**

Sequence updates on the envelope `timestamp`, which carries the time the update was
published.

**NOTE: Applying updates**

A status may be published more than once for the same order, and repeated updates need
not carry the same fields. Key your state on `order_id`, sequence on the envelope
`timestamp`, and apply the most recent update. Take fill details from the most recent
...
paths:
  ...
  /market/historical/expired/{interval}:
    get:
      tags: [Market Data]
      summary: Get historical OHLCV data for expired contracts
      description: |
